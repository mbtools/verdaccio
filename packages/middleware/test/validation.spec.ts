...
describe('validate package name middleware', () => {
  test.each(['jquery', '-'])('%s should be valid package name', async (pkg: string) => {
    const app = getApp([]);
  ...
  test.each(['node_modules', '%'])('%s should be invalid package name', async (pkg: string) => {
    const app = getApp([]);
    app.param('pkg', validatePackage);
    app.get('/:pkg', (_req, res) => {
      res.status(HTTP_STATUS.OK).json({});
  ...
  test('should validate package name double level', async () => {
    ...
    await request(app).get('/pkg/react').expect(HTTP_STATUS.OK);
  });
  ...
  test('should fails validate package name double level', async () => {
    ...
    await request(app).get('/pkg/node_modules').expect(HTTP_STATUS.BAD_REQUEST);
  });
});
...
describe('validate file name name middleware', () => {
  ...
  test.each(['some%2Fthing', '.bin'])('%s should be invalid package name', async (pkg: string) => {
    const app = getApp([]);
    app.param('pkg', validateName);
    app.get('/:pkg', (_req, res) => {
      res.status(HTTP_STATUS.OK).json({});
  ...
  test('should fails file name package name', async () => {
    ...
    await request(app).get('/file/__proto__').expect(HTTP_STATUS.BAD_REQUEST);
  });
  ...
  test('should validate file name package name', async () => {
    ...
    await request(app).get('/file/react.tar.gz').expect(HTTP_STATUS.OK);
  });
});
