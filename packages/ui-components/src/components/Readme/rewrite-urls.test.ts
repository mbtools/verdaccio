...
describe('rewrite-urls', () => {
  test('normalizeRepositoryUrl accepts common repository formats', () => {
  ...
  test('normalizeRepositoryUrl rejects non-GitHub repositories', () => {
    expect(normalizeRepositoryUrl('https://gitlab.com/owner/repo')).toBeNull();
  });
  ...
  test('toRawContentUrl builds raw.githubusercontent.com URLs', () => {
    expect(toRawContentUrl('owner', 'repo', 'img/test.svg')).toBe(
      'https://raw.githubusercontent.com/owner/repo/HEAD/img/test.svg'
    );
  });

  test('rewriteRelativeUrls rewrites img and anchor tags', () => {
  ...
  test('rewriteRelativeUrls leaves /-/ paths unchanged', () => {
